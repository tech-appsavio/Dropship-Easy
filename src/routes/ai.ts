...
import { resolveMondayToken } from '../services/account-store';
import { mondayModelsChat } from '../services/monday-models';
...
router.post('/api/ai/assistant', authenticationMiddleware, async (req, res) => {
    try {
        ...
        const boardName = String(req.body?.boardName || '').slice(0, 200);
        if (!prompt) return res.status(400).json({ error: 'prompt required' });

        const token = await resolveMondayToken(String(accountId));
        if (!token) return res.status(200).json({ answer: 'Please reconnect your monday account in Account Settings to use the assistant.' });
        ...
        const result = await mondayModelsChat(token, [
...
router.post('/api/ai/rank', authenticationMiddleware, async (req, res) => {
    try {
        const accountId = req.session?.accountId;
        if (!accountId) return res.status(401).json({ error: 'no account session' });

        const kind = req.body?.kind === 'courier' ? 'courier' : 'supplier';
        const items = Array.isArray(req.body?.items) ? req.body.items.slice(0, 25) : [];
        if (items.length < 2) return res.json({ ranking: null }); // nothing to rank

        const token = await resolveMondayToken(String(accountId));
        if (!token) return res.json({ ranking: null });
        ...
        const sys =
            `You rank ${kind} options for fulfilling a dropshipping order. Criteria: ${criteria}. ` +
        ...
        if (!result.ok) return res.json({ ranking: null });

        let parsed: any = null;
        try { parsed = JSON.parse(result.content); } catch { return res.json({ ranking: null }); }
        ...
        const sameSet = returnedIds.length === inputIds.length
            ...
            && returnedIds.every((id: string) => inputIds.includes(id));
        if (!sameSet) return res.json({ ranking: null });

        const clean = ranking.map((r: any) => ({
            id: String(r.id),
            tag: ['Best', 'Good', 'Average', 'Poor'].includes(r?.tag) ? r.tag : 'Good',
