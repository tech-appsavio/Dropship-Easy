...
interface FieldDef { key: keyof Settings; label: string; type?: string; placeholder?: string; }
...
const SECTIONS: { title: string; icon: string; hint?: string; fields: FieldDef[] }[] = [
    {
        ...
        fields: [
            ...
            { key: "whatsappTemplateLanguage", label: "Default Template Language", placeholder: "en" },
        ],
    },
    {
        title: "Shiprocket",
        icon: "📦",
        hint: "Your Shiprocket account login — used to create shipments and assign couriers.",
        fields: [
            ...
            { key: "shiprocketPickupLocation", label: "Pickup Location", placeholder: "Primary" },
        ],
    },
];
...
const Field: React.FC<{ label: string; type?: string; value: string; placeholder?: string; onChange: (v: string) => void }> = ({ label, type, value, placeholder, onChange }) => {
    ...
    return (
        <div style={{ marginBottom: 14 }}>
            ...
            <input
                ...
                style={{
                    width: "100%", boxSizing: "border-box", padding: "9px 11px", fontSize: 14, color: C.text,
                    border: `1px solid ${focus ? C.primary : C.inputBorder}`, borderRadius: 8, outline: "none",
                    boxShadow: focus ? `0 0 0 3px rgba(0,115,234,0.12)` : "none", transition: "border .15s, box-shadow .15s",
                }}
            />
        </div>
    );
